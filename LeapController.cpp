...
private:
	bool g_circle_loops_allowed;
	bool g_swipe_allowed = true;
};
...
void LeapListener::onFrame(const Controller& controller) {
	...
	for (HandList::const_iterator hl = hands.begin(); hl != hands.end(); ++hl) {
		...
		for (FingerList::const_iterator fl = fingers.begin(); fl != fingers.end(); ++fl) {
			...
			Vector w = hand.sphereCenter();
			float u = v.distanceTo(hand.sphereCenter();
			std::cout << "Distance: " << u << std::endl;
			std::cout << "Distance: " << v.toString() << std::endl;
			std::cout << "Distance: " << w.toString() << std::endl;
			if (u  > 50000){
				g_grab_allowed = false;
			}*/
		...
		if (hand.isValid() && extendedFingers<1){
			...
			//	<< "Grab "
			//	<< "Position: " <<hand.wristPosition()<< std::endl;
		}
	}
...
int main(int argc, char** argv) {
	// Create a Leaplistener and controller
	...
	// Remove the Leaplistener when done
